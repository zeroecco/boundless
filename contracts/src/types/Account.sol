...
struct Account {
    ...
    uint96 stakeBalance;
    /// @notice 80 pairs of 2 bits representing the status of a request. One bit is for lock-in and the other is for fulfillment.
    /// @dev Request state flags are packed into a uint160 to make balance and flags for the first 80 requests fit in one slot.
