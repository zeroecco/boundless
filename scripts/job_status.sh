...
echo "Effective Hz:"
query_postgres "SELECT CAST(res1.total_cycles AS decimal)  / res2.elapsed_sec AS hz, res1.total_cycles, res2.elapsed_sec FROM (SELECT output->'total_cycles' total_cycles FROM tasks WHERE task_id = 'init' AND job_id = '${JOB}') res1, (SELECT EXTRACT(EPOCH FROM (MAX(updated_at) - MIN(started_at))) AS elapsed_sec FROM tasks WHERE job_id = '${JOB}') res2;"
