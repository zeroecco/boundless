...
pub trait RequestBuilder<Params> {
    ...
    // also be used to set initial values on the params that are specific to the request builder.
...
#[non_exhaustive]
pub struct StandardRequestBuilder<P = NotProvided, S = StandardStorageProvider> {
...
impl RequestParams {
    ...
    ///
    /// ```rust
    /// # use boundless_sdk::request_builder::RequestParams;
    /// # const ECHO_ELF: &[u8] = b"";
    /// use boundless_sdk::GuestEnv;
    ///
    /// RequestParams::new()
    ...
    pub fn require_request_id(&self) -> Result<&RequestId, MissingFieldError> {
        self.request_id.as_ref().ok_or(MissingFieldError::with_hint(
            "request_id",
            "can be set using .with_request_id(...), and can be generated by boundless_sdk::Client",
        ))
    }
    ...
    ///
    /// ```rust
    /// # use boundless_sdk::request_builder::{RequestParams, OfferParams};
    ...
    ///
    /// ```rust
    /// # use boundless_sdk::request_builder::{RequestParams, RequirementParams};
...
//     use crate::{
//         contracts::{Predicate, RequestInput, RequestInputType, Requirements},
//         input::GuestEnv,
...
//             test_ctx.deployment.boundless_sdk_address,
