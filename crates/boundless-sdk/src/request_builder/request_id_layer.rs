...
impl From<RpcProvider> for RequestIdLayer {
    fn from(provider: RpcProvider) -> Self {
        RequestIdLayer { provider, config: Default::default() }
    }
}
...
impl RequestIdLayer {
    ...
    /// configured mode.
    pub fn new(provider: RpcProvider, config: RequestIdLayerConfig) -> Self {
...
impl Layer<()> for RequestIdLayer {
    ...
    async fn process(&self, (): ()) -> Result<Self::Output, Self::Error> {
        let id_u256 = match self.config.mode {
            RequestIdLayerMode::Nonce => {
                RequestId::u256(self.provider.caller(), self.provider.get_nonce().await?)
            }
            RequestIdLayerMode::Rand => RequestId::u256(self.provider.caller(), rand::random()),
        };
        Ok(id_u256.try_into().expect("generated request ID should always be valid"))
    }
}

impl Adapt<RequestIdLayer> for RequestParams {
