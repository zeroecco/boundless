...
#[allow(dead_code)]
pub type PostgresDb = DbPool<Postgres>;

const SQL_BLOCK_KEY: i64 = 0;

#[async_trait::async_trait]
impl DBPoolManager<Postgres> for DbPool<Postgres> {
    async fn new(conn_str: &str) -> Result<Self, DbError> {
...
#[cfg(test)]
impl From<Pool<Postgres>> for DbPool<Postgres> {
...
#[async_trait]
impl BrokerDb for DbPool<Postgres> {
    ...
    async fn get_order_for_pricing(&self) -> Result<Option<(U256, Order)>, DbError> {
        let elm: Option<DbOrder> = sqlx::query_as(
            ...
            SET data = 
                ...
                jsonb_set(data, 
                '{status}', to_jsonb($1)),
                '{update_at}', to_jsonb($2))
            WHERE id =
                (SELECT id
                FROM orders
                WHERE data->>'status' = $3::text
                LIMIT 1)
            RETURNING *
            "#,
        )
    ...
    async fn get_active_pricing_orders(&self) -> Result<Vec<(U256, Order)>, DbError> {
        let orders: Vec<DbOrder> =
            sqlx::query_as("SELECT * FROM orders WHERE data->>'status' = $1::text")
                .bind(OrderStatus::Pricing)
                .fetch_all(&self.pool)
                .await?;
    ...
    async fn set_order_lock(
        ...
        let res = sqlx::query(
            ...
            SET data = 
                ...
                jsonb_set(data, 
                '{status}', to_jsonb($1)),
                '{target_timestamp}', to_jsonb($2)),
                '{expire_timestamp}', to_jsonb($3)),
    ...
    async fn set_proving_status(&self, id: U256, lock_price: U256) -> Result<(), DbError> {
        let res = sqlx::query(
            ...
            SET data = 
                    ...
                    jsonb_set(data, 
                    '{status}', to_jsonb($1)),
                    '{updated_at}', to_jsonb($2)),
                    '{lock_price}', to_jsonb($3))
            WHERE
                id = $4"#,
        )
    ...
    async fn get_pending_lock_orders(
        ...
        let orders: Vec<DbOrder> = sqlx::query_as(
            "SELECT * FROM orders WHERE data->>'status' = $1::text AND data->>'target_timestamp' <= $2::text",
        )
        .bind(OrderStatus::Locking)
    ...
    async fn set_image_input_ids(
        ...
        let res = sqlx::query(
            ...
            SET data = jsonb_set(
                       jsonb_set(
                       jsonb_set(data, 
                       '{image_id}', to_jsonb($1::text)),
                       '{input_id}', to_jsonb($2::text)),
                       '{updated_at}', to_jsonb($3::bigint)
                )
            WHERE
                id = $4"#,
        )
    ...
    async fn complete_batch(&self, batch_id: usize, g16_proof_id: String) -> Result<(), DbError> {
        ...
        let res = sqlx::query(
            ...
            SET data = 
                ...
                jsonb_set(data, 
                '{status}', to_jsonb($1)),
                '{aggregation_state,groth16_proof_id}', to_jsonb($2))
            WHERE
                id = $3"#,
        )
    ...
    async fn get_complete_batch(&self) -> Result<Option<(usize, Batch)>, DbError> {
        let elm: Option<DbBatch> = sqlx::query_as(
            ...
            UPDATE batches
            SET
                data = jsonb_set(data, '{status}', to_jsonb($1))
            WHERE id =
                (SELECT id
                FROM batches
                WHERE data->>'status' = $2::text
                LIMIT 1)
            RETURNING *
            "#,
        )
    ...
    async fn get_current_batch(&self) -> Result<usize, DbError> {
        ...
        if batch_count == 0 {
            ...
            let cur_batch: Option<DbBatch> = sqlx::query_as(
                "SELECT * FROM batches WHERE data->>'status' IN ($1::text, $2::text) LIMIT 1",
            )
    ...
    async fn update_batch(
        ...
        let db_deadline_str: Option<String> = rows.try_get("deadline")?;
        let db_deadline: Option<i64> = if let Some(db_deadline) = db_deadline_str {
            Some(i64::from_str(&db_deadline).unwrap())
        } else {
            None
        };
            ...
            let res = sqlx::query(
                ...
                SET
                    data = 
                        jsonb_set(data,
                        '{orders}',coalesce(
                            (data->'orders') || to_jsonb($1::text), 
                            jsonb_build_array(to_jsonb($1::text))
                        ))
                WHERE
                    id = $2"#,
            )
...
mod tests {
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_order(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
        let id = U256::ZERO;
        let order = create_order();
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_submission_order(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
        let id = U256::ZERO;
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_order_for_pricing(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
        let id = U256::ZERO;
        let order = create_order();
        db.add_order(id, order.clone()).await.unwrap();
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_active_pricing_orders(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn set_order_lock(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
        let id = U256::ZERO;
        let order = create_order();
        db.add_order(id, order.clone()).await.unwrap();
    ...
    async fn set_order_lock_fail(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
        let id = U256::ZERO;
        let order = create_order();
        db.add_order(id, order.clone()).await.unwrap();
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn set_proving_status(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
        let id = U256::ZERO;
        let order = create_order();
        db.add_order(id, order.clone()).await.unwrap();
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn set_order_failure(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
        let id = U256::ZERO;
        let order = create_order();
        db.add_order(id, order.clone()).await.unwrap();
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn set_order_complete(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
        let id = U256::ZERO;
        let order = create_order();
        db.add_order(id, order.clone()).await.unwrap();
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn skip_order(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
        let id = U256::ZERO;
        let order = create_order();
        db.add_order(id, order.clone()).await.unwrap();
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_pending_lock_orders(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_proving_order(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn set_order_proof_id(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_active_proofs(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn set_image_input_ids(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn set_aggregation_status(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_aggregation_proofs(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_current_batch(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn add_batch(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn complete_batch(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn get_complete_batch(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn set_batch_submitted(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn set_batch_failure(pool: PgPool) {
        let db: DbObj = Arc::new(PostgresDb::from(pool));
    ...
    #[sqlx::test(migrations = "./migrations/postgres")]
    async fn update_batch(pool: PgPool) {
