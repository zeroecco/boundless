...
impl From<Requirements> for RequirementParams {
    fn from(value: Requirements) -> Self {
        Self {
            ...
            callback_gas_limit: Some(value.callback.gas_limit.to()),
        }
    }
}
...
impl TryFrom<RequirementParams> for Requirements {
    ...
    fn try_from(value: RequirementParams) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            image_id: value.image_id.ok_or(MissingFieldError::with_hint(
                "image_id",
            ...
            callback: Callback {
                ...
                gas_limit: U96::from(value.callback_gas_limit.unwrap_or_default()),
            },
        })
    }
}
    ...
    async fn process(
        ...
        let callback = params
            ...
            .map(|addr| Callback {
                addr,
                gas_limit: U96::from(
                    params.callback_gas_limit.unwrap_or(DEFAULT_CALLBACK_GAS_LIMT),
                ),
            })
            .unwrap_or_default();
