    ...
    Claim: Digestible + Debug + Clone + Serialize,
{
    ...
    pub fn with_root(self, root_receipt: Receipt) -> Self {
        Self { root: Some(root_receipt), ..self }
    }
    ...
    #[cfg(feature = "verify")]
    pub fn verify_integrity(&self) -> Result<(), anyhow::Error> {
    ...
    pub fn verify_integrity_with_context(
        ...
        anyhow::bail!("failed to confirm the validity the set root: {}", path_root);
    }
...
impl Digestible for SetInclusionReceiptVerifierParameters {
    ...
    fn digest<S: Sha256>(&self) -> Digest {
        tagged_struct::<S>("risc0.SetInclusionReceiptVerifierParameters", &[self.image_id], &[])
    }
}
...
pub enum GuestInput {
    ...
    Singleton { self_image_id: Digest, claim: ReceiptClaim },
    /// Input is an inner node of the Merkle tree.
    Join { self_image_id: Digest, left_set_root: Digest, right_set_root: Digest },
}
...
impl GuestInput {
    ...
    pub fn root(&self) -> Digest {
        match self {
            ...
            GuestInput::Join { left_set_root, right_set_root, .. } => {
...
pub fn merkle_root(leaves: &[Digest]) -> Result<Digest, Error> {
    match leaves {
        [] => Err(anyhow!("Digest list is empty, cannot compute Merkle root")),
        [digest] => Ok(*digest), // If only one digest, return it as the root
        _ => {
            ...
            Ok(commutative_keccak256(&left_root, &right_root))
        }
    }
}
...
pub fn merkle_path(leaves: &[Digest], index: usize) -> Result<Vec<Digest>, Error> {
    // Ensure that the index is valid.
    leaves
        .get(index)
        .ok_or(anyhow!("no leaf with index {index} in tree of size {}", leaves.len()))?;

    match leaves {
        [] => Err(anyhow!("Digest list is empty, cannot compute Merkle path")),
        [_] => Ok(Vec::new()), // If only one digest, return an empty path.
        _ => {
...
pub fn merkle_path_root(
    ...
    path.into_iter().fold(*leaf, |a, b| commutative_keccak256(a.borrow(), b.borrow()))
}
...
impl GuestOutput {
    ...
    pub fn new(image_id: impl Into<Digest>, root: Digest) -> Self {
        Self { id: to_b256(image_id.into()), root: to_b256(root) }
    }
...
mod tests {
    ...
    fn test_merkle_root() {
        ...
        let digests =
            vec![Digest::from([0u8; 32]), Digest::from([1u8; 32]), Digest::from([2u8; 32])];
        assert_merkle_root(
            &digests,
            commutative_keccak256(&commutative_keccak256(&digests[0], &digests[1]), &digests[2]),
        );
    ...
    fn test_consistency() {
        for length in 1..=256 {
            let digests: Vec<Digest> =
                (0..length).map(|_| rand::random::<[u8; 32]>().into()).collect();
            let root = merkle_root(&digests).unwrap();

            for i in 0..length {
                let path = merkle_path(&digests, i).unwrap();
