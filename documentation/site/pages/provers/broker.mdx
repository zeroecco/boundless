...
```sh [Terminal]
# For example, to benchmark an order on testnet
source <(just env testnet)
...
This prevents the broker from locking new orders while you prepare for the upgrade.

:::note[Bind Mount Issue]
If the bind mount drops and the file update is not reflected in the broker, you can try to modify the file in docker compose, for example:

```sh [Terminal]
docker compose exec broker sh -c 'sed "s/^[[:space:]]*#*[[:space:]]*max_concurrent_proofs[[:space:]]*=[[:space:]]*[0-9]*/max_concurrent_proofs = 0/" /app/broker.toml > /tmp/broker_temp.toml && cp /tmp/broker_temp.toml /app/broker.toml'
```
:::

##### Wait for in-progress orders to complete

Monitor your broker logs (or the [Boundless Explorer](https://explorer.beboundless.xyz)) to ensure all currently locked orders are completed. This may take some time depending on the state of the committed orders.
...
##### Reset the max_concurrent_proofs configuration
...
# Example: max_concurrent_proofs = 4
```
