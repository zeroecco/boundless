...
:::note[Source Code]
The *Sending a Request* section uses the counter example as a template, its source code can be found at: [apps/src/main.rs](https://github.com/boundless-xyz/boundless/blob/main/examples/counter/apps/src/main.rs)
:::
...
let client = Client::builder()
  ...
  .with_storage_provider(storage_provider_from_env()?)?
  .build()
  .await?;
# Ok(())
# }
...
let client = Client::builder()
  .with_rpc_url(args.rpc_url)
  .with_private_key(args.private_key)
  .with_storage_provider(storage_provider_from_env()?)? // [!code hl] // [!code focus]
  .build()
  .await?;
# Ok(())
# }
...
If you are working in a monorepo, you can take advantage of [risc0-build](https://docs.rs/risc0-build/latest/risc0_build/) which automatically builds and exposes the ELF for the guest. The counter example uses this method:
...
# program: &'static [u8],
# input: &[u8]) -> Result<()> {
// In a real application, you would import ECHO_ELF from your guest code:
...
[Offer parameters](/developers/tutorials/pricing#contents-of-an-offer) allow you to fine tune the pricing, auction, staking and expiry parameters for your Boundless request.

There are two ways to configure offer parameters:

1. Using `with_offer` on individual requests to override parameters for a specific request.
2. Using `config_offer_layer` on the client builder to configure pricing logic the SDK uses to automatically calculate offers.
...
let client = Client::builder()
  ...
  .config_offer_layer(|config| config
    ...
    .lock_timeout(120)
    .timeout(300)
    // Configure gas estimates for more accurate pricing
    .lock_gas_estimate(200_000)
    .fulfill_gas_estimate(750_000)
  )
  .build()
  .await?;
# Ok(())
...
With this configuration, when you create requests and provide cycle count estimates, the SDK will automatically calculate appropriate prices:
...
# program: &'static [u8],
# input: &[u8]) -> Result<()> {
// The SDK will automatically calculate prices based on the cycle count
let request = client.new_request()
  .with_program(program)
  .with_stdin(input)
  .with_cycles(1_000_000); // Provide cycle count for automatic pricing

// The offer will be calculated using:
// - min_price = min_price_per_cycle * cycle_count
// - max_price = max_price_per_cycle * cycle_count + gas_cost_estimate
let (request_id, expires_at) = client.submit_onchain(request).await?;
# Ok(())
# }
```
...
- **Use `with_offer`** when:
  ...
  - You want to set exact prices rather than using cycle-based calculations
