...
1. Program Image (`with_program`) (line #2)
2. Program Input (`with_stdin`) (line #3)
3. Offer Settings (`with_offer`) (line #4)
...
# async fn submit_request(client: Client, request: ProofRequest) -> Result<(), Box<dyn std::error::Error>> {
...
Using `client.submit_offchain(request)`, you can send your request offchain through the order-stream service.
...
# use boundless_market::Client;
# use boundless_market::contracts::ProofRequest;
# async fn submit_request_with_signer(client: Client, request: ProofRequest, signer: impl Signer) -> Result<(), Box<dyn std::error::Error>> {
let (request_id, expires_at) = client.submit_request_onchain_with_signer(&request, &signer).await?;
# Ok(())
# }
```
</StripRustCodeComments>

Similarly, using `client.submit_request_onchain_with_signer`, will send your request offchain through the order-stream service.
...
```sh [Terminal]
cargo install --path ./crates/boundless-cli --bin boundless
```
