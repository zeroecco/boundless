...
2. They allow for off-chain complex interactions like indexing, filtering, querying, etc.
...
- **Secure Bridging of Off-Chain Logic:** Bring existing off-chain analytics safely on-chain, without modifying your smart contract logic or storage patterns.
...
This unlocks sophisticated, trustless interactions using Ethereum’s inexpensive event logs as secure, verifiable on-chain inputs.
...
2. Convert the input into an `EvmEnv` for execution, with the correct chain configuration: `let env = input.into_env().with_chain_spec(&ETH_SEPOLIA_CHAIN_SPEC);`
