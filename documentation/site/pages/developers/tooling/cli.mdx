...
Alternatively, you can set the environment variables `RPC_URL` and `PRIVATE_KEY`, to avoid passing them each time.
...
Deposits HP (hitpoints) into the market as a stake (HP is used for trust/staking in Boundless):
