...
<a href="/bento_technical_diagram.png" class="lightgallery">
...
We recommend a high memory node for the Redis container as well as active monitoring / alerts (see [monitoring](/prove/monitoring) for more details) on the Redis node to ensure it does not overflow the possible memory.
:::
...
The performance optimization guide has a whole section on [segment size benchmarking](/prove/performance-optimization#finding-the-maximum-segment_size-for-gpu-vram).
