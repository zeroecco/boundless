...
The Boundless Foundry Template provides a full code example of this workflow in [`apps/src/main.rs`](https://github.com/boundless-xyz/boundless-foundry-template/blob/main/apps/src/main.rs).
...
Below is an exmaple of a `request.yaml` file that can be used with the `boundless-cli submit-request` command.
...
offer:
  minPrice: 100000000000000 # 0.0001 Sepolia ETH
...
```sh [Terminal]
# [!code word:--offchain]
RUST_LOG=info boundless-cli deposit 0.002 # Enough for the request above; you can deposit more at a time if you wish.
