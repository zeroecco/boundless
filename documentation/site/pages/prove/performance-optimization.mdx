...
Operating competing workloads can lead to unpredictable results. We recommend isolating your test system from other workloads to ensure that the performance tuning results are consistent and reliable. This includes stopping configured [Broker](/prove/broker#stopping-the-broker-service) services:
...
If you intend to operate across a variety of different workloads (such as those that may be fed by the [Broker](/prove/broker)), you can also use the following command to generate a synthetic workload:
...
:::tip[Note]
Boundless should be restarted upon changing the `SEGMENT_SIZE` value, and verify that [Broker](/prove/broker#stopping-the-broker-service) is not running.
:::
